...
def get_data(query1):
    ...
    elif query1 == 'Number of Repos In Different PRs States':
              return pd.read_json('https://node-api.flipsidecrypto.com/api/v2/queries/a7c1266f-571f-45eb-b0c2-f95ddea9b8b1/data/latest')	
    return None
...
with c1:
     fig = px.pie(df, values='Repos', names='State', title='Share of Repositories In Different Pull Requests States')
...
with subtab_Daily:
            df = Number_of_New_Repositories_per_Day
            fig = sp.make_subplots(specs=[[{'secondary_y': True}]])
            fig.add_trace(go.Bar(x=df['Day'], y=df['New Repository'], name='New Repository'), secondary_y=False)
            fig.add_trace(go.Line(x=df['Day'], y=df['Cummulative New Repository'], name='Cummulative New Repository'), secondary_y=True)
            fig.update_layout(title_text='Number of New Repository(Repo)')
            ...
            fig = sp.make_subplots(specs=[[{'secondary_y': True}]])
            fig.add_trace(go.Bar(x=df['Day'], y=df['Active Repo'], name='Active Repo'), secondary_y=False)
            fig.add_trace(go.Line(x=df['Day'], y=df['Cummulative Active Repo'], name='Cummulative Active Repo'), secondary_y=True)
            fig.update_layout(title_text='Number of Active Repositories')
            fig.update_yaxes(title_text='', secondary_y=False)
            fig.update_yaxes(title_text='', secondary_y=True)
            st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)	
...
# -------------------------------------------------Weekly------------------------------------------------------------------------------------------
with subtab_Weekly:
            df = Number_of_New_Repositories_per_Week
            fig = sp.make_subplots(specs=[[{'secondary_y': True}]])
            fig.add_trace(go.Bar(x=df['Week'], y=df['New Repository'], name='New Repository'), secondary_y=False)
            fig.add_trace(go.Line(x=df['Week'], y=df['Cummulative New Repository'], name='Cummulative New Repository'), secondary_y=True)
            fig.update_layout(title_text='Number of New Repository(Repo)')
            fig.update_yaxes(title_text='', secondary_y=False)
            fig.update_yaxes(title_text='', secondary_y=True)
            st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
            ...
            fig = sp.make_subplots(specs=[[{'secondary_y': True}]])
            fig.add_trace(go.Bar(x=df['Week'], y=df['Active Repo'], name='Active Repo'), secondary_y=False)
            fig.add_trace(go.Line(x=df['Week'], y=df['Cummulative Active Repo'], name='Cummulative Active Repo'), secondary_y=True)
            fig.update_layout(title_text='Number of Active Repositories')
            fig.update_yaxes(title_text='', secondary_y=False)
            fig.update_yaxes(title_text='', secondary_y=True)
            st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)	
...
# --------------------------------------------------Monthly----------------------------------------------------------------------------------------
with subtab_Monthly:
            df = Number_of_New_Repositories_per_Month
            fig = sp.make_subplots(specs=[[{'secondary_y': True}]])
            fig.add_trace(go.Bar(x=df['Month'], y=df['New Repository'], name='New Repository'), secondary_y=False)
            fig.add_trace(go.Line(x=df['Month'], y=df['Cummulative New Repository'], name='Cummulative New Repository'), secondary_y=True)
            fig.update_layout(title_text='Number of New Repository(Repo)')
            fig.update_yaxes(title_text='', secondary_y=False)
            fig.update_yaxes(title_text='', secondary_y=True)
            st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
            ...
            fig = sp.make_subplots(specs=[[{'secondary_y': True}]])
            fig.add_trace(go.Bar(x=df['Month'], y=df['Active Repo'], name='Active Repo'), secondary_y=False)
            fig.add_trace(go.Line(x=df['Month'], y=df['Cummulative Active Repo'], name='Cummulative Active Repo'), secondary_y=True)
            fig.update_layout(title_text='Number of Active Repositories')
            fig.update_yaxes(title_text='', secondary_y=False)
            fig.update_yaxes(title_text='', secondary_y=True)
            st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)	
...
with subtab_Yearly:
            ...
            df = Number_of_New_Repositories_per_Year
            c1, c2 = st.columns(2)
             
            with c1:
                fig = px.pie(df, values='New Repository', names='Year', title='Share of New Repository in Each Year')
                fig.update_layout(legend_title='Year', legend_y=0.5)
                fig.update_traces(textinfo='percent', textposition='inside')
                st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
		
            with c2:
                fig = px.bar(df, x='Year', y='New Repository', color='Year', title='Total Number of New Developers', log_y=False)
                fig.update_layout(showlegend=False, xaxis_title=None, legend_title='', yaxis_title='Repo Count', xaxis={'categoryorder':'total ascending'})
                st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
...
with subtab_Yearly:
            ...
            with c1:
                fig = px.pie(df, values='Active Repo', names='Year', title='Share of Active Repositories in Each Year')
                fig.update_layout(legend_title='Year', legend_y=0.5)
                fig.update_traces(textinfo='percent', textposition='inside')
                st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
		
            with c2:
                fig = px.bar(df, x='Year', y='Active Repo', color='Year', title='Total Number of Active Repositories', log_y=False)
                fig.update_layout(showlegend=False, xaxis_title=None, legend_title='', yaxis_title='Repo Count', xaxis={'categoryorder':'total ascending'})
                st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)
